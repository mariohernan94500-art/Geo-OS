...
// Carga la visión maestra del ecosistema
const NUCLEUS_PATH = 'C:/Users/conta/OneDrive/Desktop/Geo/00_nucleus/NUCLEO_DE_MARIO_v1.md';
let masterNucleus = '';
try {
    masterNucleus = fs.readFileSync(NUCLEUS_PATH, 'utf-8');
} catch(e) {
    console.warn('[BaseAgent] ⚠️ No se encontró NUCLEO_DE_MARIO_v1.md en la ruta especificada.');
}
...
export abstract class BaseAgent {
    ...
    public async delegate(context: string, userId: string): Promise<string> {
        ...
        // Inyectando el núcleo universal antes de las reglas específicas del agente
        const fullSystemPrompt = `=== CONTEXTO MAESTRO (NÚCLEO) ===\n${masterNucleus}\n\n=== REGLAS DEL AGENTE ESPECÍFICO ===\n${this.systemPrompt}`;

        const mensajesParaAgente = [
            { role: 'system', content: fullSystemPrompt } as any,
