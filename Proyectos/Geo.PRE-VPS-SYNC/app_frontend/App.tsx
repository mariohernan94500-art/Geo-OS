...
// ─── Auth & Config ───────────────────────────────────────────────────────────
// DEV_TOKEN eliminado: el token ahora vive en SecureStore (Keychain/EncryptedSharedPreferences).
// BASE_URL eliminado: se lee de app.config.js → expo.extra.apiUrl via src/config.ts
...
// ─── CONSTANTE BASE_URL resuelta desde config central ────────────────────────
...
// ─── APP ─────────────────────────────────────────────────────────────────────
function App() {
  // ── Auth: token leído de SecureStore ─────────────────────────────────────
  // isAuthenticated: null = cargando, false = sin sesión, true = con sesión
  const [isAuthenticated, setIsAuthenticated] = useState<boolean | null>(null);
  const sessionTokenRef = useRef<string | null>(null);

  useEffect(() => {
    // Verificar si ya existe un token guardado al arrancar
    getToken().then((token) => {
  ...
  const procesarInteraccion = async (task: InteractionTask) => {
    ...
    try {
      ...
      // Token leído de SecureStore en cada request (nunca del bundle)
      const authToken = sessionTokenRef.current ?? '';
  ...
  const handleSendText = async () => {
    ...
    setIsLoading(true);
    try {
      // Token leído de SecureStore (nunca hardcodeado en el bundle)
      const authToken = sessionTokenRef.current ?? '';
  ...
  if (isAuthenticated === null) {
    // Cargando token desde SecureStore
    return (
      <SafeAreaProvider>
  ...
  if (!isAuthenticated) {
    // Sin sesión → pantalla de login placeholder
    // PENDIENTE: reemplazar por flujo real antes de distribución pública
    return (
      <SafeAreaProvider>
